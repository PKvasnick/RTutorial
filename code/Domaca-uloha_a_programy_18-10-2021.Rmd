...
Úlohu s integráciou ste odbyli tým, že ste do svojej funkcie zabalili štandardnú funkciu R. Žiadalo sa niečo naprogramovať vlastnými silami, hoci aj jednoduché. 
...
## Programujeme
Treba si trocha zašpiniť ruky s R, s peknučkým rozprávaním sa nikam nedostaneme. 

1. Vypísať všetky prvočísla menšie alebo rovné n. 
Postup: Erastothenovo sito. Pracujeme nad logickým poľom o veľkosti n a postupne vyškrtávame násobky prvočísel. 
```{r}
...
v
```
Triedenie opakovaným výberom minima
...
Triedenie použitím R

```{r}
v = seq(from = 10, to = 1, by = -1)
sort(v)
```
...
```{r}
l <- 20
v <- seq(from = -10, to = 10, length.out = l)

N <- 1000
M <- matrix(nrow = N, ncol = l)
for(i in 1:N)
  M[i,] <- sample(v, size = l, replace = TRUE)
