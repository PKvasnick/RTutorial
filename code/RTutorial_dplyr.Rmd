...
flights
```
Toto nie je R `data.frame`, ale `tibble`, trocha od2ahčená a zmodernizovaná verzia tabuľky vytvorený pre `tidyverse`. Rozdiely nás zatiaě nebudú zaujímať.
...
R buď vytlačí výslednú hodnotu posledného príkazu, alebo ju priradí. Ak chcete oboje, dajte príkaz do zátvoriek. Ak nechcete ani jedno, ukončite bodkočiarkou:

```{r}
(dec25 <- filter(flights, month == 12, day == 25))
```

Provnania v R: štandardný súbor: `< <= == != >= >`. Častá chyba našastie dáva chybovú hlášku:
