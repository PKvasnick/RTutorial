...
__ggplot2__ je výnimoèný svojou funkcionalitou, kvalitou výstupu a osobitou filozofiou. Ako so v¹etkým v R, budete si chvíµu zvyka», ale sµubujem, ¾e to, èo získate na prvý pokus, bude v¾dy o niekoµko tried lep¹ie ako prvý pokus o graf v Exceli.

```{r}
library(tidyverse)
```

Neznepokojujeme sa výstrahami o konfliktoch. *tidyverse* predefinuje niektoré veci v R, ale to nás nemusí znepokojova». 
...
1. Spustite `ggplot(data = mpg)` bez `geom_xxxx()`. Èo vidíte?
...
E¹te jeden spôsob, ako prida» do grafu ïal¹ie premenné, je rozdeli» graf do viacerých "faziet" a v ka¾dej zobrazi» iba podmno¾inu dát. Pre takéto delenie je vhodné pouu¾iø kategoriálne dáta.
